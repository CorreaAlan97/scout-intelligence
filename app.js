...
const STATS_URL = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?tqx=out:json&sheet=Stats%20Sofascore`;
...
async function loadData() {
  setStatus('loading');
  try {
    const [vRes, sRes] = await Promise.all([fetch(SHEET_URL), fetch(STATS_URL)]);
    const vText = await vRes.text();
    const sText = await sRes.text();
 
    // Visorias
    const vJson = JSON.parse(vText.match(/google\.visualization\.Query\.setResponse\(([\s\S]*)\)/)[1]);
    const vCols = vJson.table.cols.map(c => c.label);
    const vRows = (vJson.table.rows || []).map(r => parseRow(r, vCols));
    allRows = vRows.filter(r => {
    ...
    // Stats Sofascore
    try {
      const sJson = JSON.parse(sText.match(/google\.visualization\.Query\.setResponse\(([\s\S]*)\)/)[1]);
      const sCols = sJson.table.cols.map(c => c.label);
      const sRows = (sJson.table.rows || []).map(r => parseRow(r, sCols));
      sofascoreStats = {};
      sRows.forEach(r => {
        const nombre = (r['Jugador'] || '').toString().trim().toLowerCase();
        if (nombre) sofascoreStats[nombre] = r;
      });
    } catch(e) {
      sofascoreStats = {};
    }
...
function getSofascoreData(nombre) {
  return sofascoreStats[nombre.toLowerCase().trim()] || null;
}
...
function groupByPlayer(rows) {
  ...
  rows.forEach(row => {
    ...
    if (!map[key]) {
      map[key] = { nombre, visorias: [], pos: '', rol: '', equipo: '', edad: '', nac: '', nivelGeneral: null, nivelPotencial: null, conNivel: false };
    }
  ...
  return Object.values(map).map(pg => {
    ...
    const niveles = pg.visorias.map(v => parseFloat(getField(v, 'habilidad general', 'nivel general'))).filter(n => !isNaN(n));
    pg.nivelGeneral = niveles.length ? +(niveles.reduce((a, b) => a + b, 0) / niveles.length).toFixed(1) : null;
    const potenciales = pg.visorias.map(v => parseFloat(getField(v, 'habilidad potencial', 'nivel potencial'))).filter(n => !isNaN(n));
    pg.nivelPotencial = potenciales.length ? +(potenciales.reduce((a, b) => a + b, 0) / potenciales.length).toFixed(1) : null;
    pg.conNivel = pg.visorias.some(v => { const nv = getField(v, 'nivel para', 'nivel para el').toLowerCase(); return nv === 'sí' || nv === 'si'; });
    return pg;
  });
}
...
function showError() {
  document.getElementById('metricsGrid').innerHTML = `<div class="empty-state" style="grid-column:1/-1"><div class="empty-icon">⚠</div><p>No se pudo conectar al Google Sheet.</p></div>`;
}
...
function renderOverview() {
  ...
  document.getElementById('metricsGrid').innerHTML = `
    <div class="metric-card"><div class="metric-label">Jugadores</div><div class="metric-value">${total}</div><div class="metric-sub">${totalVisorias} visorias en total</div></div>
    <div class="metric-card"><div class="metric-label">Nivel promedio</div><div class="metric-value metric-accent">${prom}</div><div class="metric-sub">habilidad general</div></div>
    <div class="metric-card"><div class="metric-label">Con nivel</div><div class="metric-value">${conNivel}</div><div class="metric-sub">aptos para captación</div></div>
    <div class="metric-card"><div class="metric-label">Posiciones</div><div class="metric-value">${posSet.size}</div><div class="metric-sub">perfiles distintos</div></div>
  `;
...
function renderOverviewCharts() {
  const posCounts = {};
  playerGroups.forEach(pg => { const pos = pg.pos || 'Sin datos'; posCounts[pos] = (posCounts[pos] || 0) + 1; });
  ...
  if (posCtx) {
    charts.pos = new Chart(posCtx, {
      type: 'doughnut',
      data: { labels: posLabels, datasets: [{ data: posLabels.map(l => posCounts[l]), backgroundColor: colors.slice(0, posLabels.length), borderWidth: 0, hoverOffset: 4 }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { color: '#9898a8', font: { size: 11, family: 'DM Mono' }, padding: 8, boxWidth: 10 } } } }
    });
  }
  const nivCounts = {};
  playerGroups.forEach(pg => { if (pg.nivelGeneral !== null) { const n = pg.nivelGeneral.toString(); nivCounts[n] = (nivCounts[n] || 0) + 1; } });
  ...
  if (nivCtx) {
    charts.niv = new Chart(nivCtx, {
      type: 'bar',
      data: { labels: nivLabels, datasets: [{ data: nivLabels.map(l => nivCounts[l]), backgroundColor: '#c8f04a', borderRadius: 3, borderWidth: 0 }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { ticks: { color: '#55555f', font: { size: 11 } }, grid: { color: '#2a2a35' } }, x: { ticks: { color: '#55555f', font: { size: 11 }, maxRotation: 45 }, grid: { display: false } } } }
    });
  }
}
...
function renderPlayers() {
  ...
  if (filtered.length === 0) { grid.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><div class="empty-icon">🔍</div><p>No hay jugadores con esos filtros.</p></div>'; return; }
  grid.innerHTML = filtered.map(pg => {
    const hasSofascore = !!getSofascoreData(pg.nombre);
    return `
    <div class="player-card ${selectedPlayerName === pg.nombre ? 'selected' : ''}" onclick="selectPlayerByName('${pg.nombre.replace(/'/g,"\\'")}')">
      ...
      <div style="margin-top:6px;display:flex;gap:4px;flex-wrap:wrap">
        ${pg.conNivel ? '<span class="badge badge-green">Con nivel</span>' : ''}
        ${hasSofascore ? '<span class="badge badge-blue">Stats</span>' : ''}
      </div>
    </div>`;
  }).join('');
}
...
function renderDetail() {
  ...
  const valoraciones = pg.visorias.map(v => parseFloat(getField(v, 'valoración', 'valoracion'))).filter(n => !isNaN(n));
  const valoracionProm = valoraciones.length ? (valoraciones.reduce((a, b) => a + b, 0) / valoraciones.length).toFixed(1) : null;
  const sf = getSofascoreData(pg.nombre);
  ...
  document.getElementById('detailContent').innerHTML = `
    <div class="detail-header">
      ...
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px">
        ...
        ${sf ? '<span class="badge badge-blue">Stats Sofascore</span>' : ''}
    ...
    <div class="detail-grid">
      ...
      <div class="detail-card">
        ...
        ${!pg.nivelGeneral && !pg.nivelPotencial && !valoracionProm ? '<div style="color:var(--text-3);font-size:13px">Sin métricas numéricas.</div>' : ''}
      </div>
    </div>
 
    ${sf ? renderSofascoreBlock(sf) : ''}
 
    ${desc ? `<div class="obs-block" style="margin-bottom:12px"><div class="obs-title">Descripción del jugador</div><div class="obs-item-text">${desc}</div></div>` : ''}
    ...
    <div class="obs-block">
      ...
      ${pg.visorias.map((v, i) => {
        ...
        const obsItems = [['Obs. técnicas ofensivas', otec],['Obs. técnicas defensivas', dtec],['Obs. tácticas ofensivas', otac],['Obs. tácticas defensivas', dtac],['Obs. físicas', fis],['Obs. mentales / actitudinales', men]].filter(([, v]) => v);
        return `
          <div class="obs-item">
            ...
            ${obsItems.map(([l, v]) => `<div style="margin-bottom:6px"><div style="font-size:11px;color:var(--accent);font-family:var(--font-mono);margin-bottom:2px">${l}</div><div style="font-size:13px;color:var(--text-2);line-height:1.6">${v}</div></div>`).join('')}
...
// ============================================
// SOFASCORE BLOCK — renderiza según posición
// ============================================
 
function sfVal(sf, key) {
  const v = sf[key];
  if (v === null || v === undefined || v === '') return null;
  return v.toString().trim();
}
 
function sfNum(sf, key) {
  const v = sfVal(sf, key);
  return v !== null ? parseFloat(v) : null;
}
 
function renderSofascoreBlock(sf) {
  const pos = (sf['Posición'] || '').toString().toLowerCase();
  const isArquero = pos.includes('arquero');
  const isDelantero = pos.includes('delantero') || pos.includes('extremo');
  const isMedioCampo = pos.includes('volante') || pos.includes('mediocampista') || pos.includes('interno') || pos.includes('enganche');
  const isDefensor = pos.includes('lateral') || pos.includes('central') || pos.includes('líbero') || pos.includes('libero');
 
  // Métricas generales siempre presentes
  const generales = [
    ['Partidos', sfVal(sf, 'Partidos')],
    ['Titularidades', sfVal(sf, 'Titularidades') ? `${sfVal(sf, 'Titularidades')} (${sfVal(sf, 'Titularidades %')})` : null],
    ['Minutos jugados', sfVal(sf, 'Minutos jugados')],
    ['Rating', sfVal(sf, 'Rating')],
    ['Toques por partido', sfVal(sf, 'Toques')],
    ['Pases precisos %', sfVal(sf, 'Pases precisos %')],
  ].filter(([, v]) => v);
  ...
  if (isArquero) {
    especificas = [
      ['Atajadas / partido', sfVal(sf, 'Atajadas')],
      ['Atajadas cercanas / partido', sfVal(sf, 'Atajadas cercanas')],
      ['Goles concedidos', sfVal(sf, 'Goles concedidos')],
      ['Goals prevented / partido', sfVal(sf, 'Goals prevented')],
      ['Vallas invictas', sfVal(sf, 'Vallas invictas')],
      ['Pases largos precisos %', sfVal(sf, 'Pases largos precisos')],
      ['Reclamos altos / partido', sfVal(sf, 'Reclamos altos')],
      ['Penales enfrentados', sfVal(sf, 'Penales enfrentados')],
      ['Penales atajados', sfVal(sf, 'Penales atajados')],
    ].filter(([, v]) => v);
    bars = [
      [sfNum(sf, 'Atajadas'), 'Atajadas / partido', 10, '#c8f04a'],
      [sfNum(sf, 'Vallas invictas'), 'Vallas invictas', sfNum(sf, 'Partidos') || 30, '#4af0c8'],
      [sfNum(sf, 'Rating'), 'Rating', 10, '#4a90f0'],
    ];
  } else if (isDefensor || isMedioCampo) {
    especificas = [
      ['Duelos ganados %', sfVal(sf, 'Duelos ganados %')],
      ['Duelos aéreos ganados %', sfVal(sf, 'Duelos aéreos ganados %')],
      ['Recuperaciones / partido', sfVal(sf, 'Recuperaciones')],
      ['Intercepciones / partido', sfVal(sf, 'Intercepciones')],
      ['Quites / partido', sfVal(sf, 'Quites')],
      ['Pases clave / partido', sfVal(sf, 'Pases clave')],
      ['Centros / partido', sfVal(sf, 'Centros')],
      ['Faltas cometidas / partido', sfVal(sf, 'Faltas cometidas')],
      ['Amarillas', sfVal(sf, 'Amarillas')],
      ['Rojas', sfVal(sf, 'Rojas')],
    ].filter(([, v]) => v);
    bars = [
      [sfNum(sf, 'Duelos ganados %'), 'Duelos ganados %', 100, '#c8f04a'],
      [sfNum(sf, 'Recuperaciones'), 'Recuperaciones / partido', 15, '#4af0c8'],
      [sfNum(sf, 'Intercepciones'), 'Intercepciones / partido', 5, '#4a90f0'],
      [sfNum(sf, 'Rating'), 'Rating', 10, '#f0c84a'],
    ];
  } else if (isDelantero) {
    especificas = [
      ['Goles', sfVal(sf, 'Goles')],
      ['Asistencias', sfVal(sf, 'Asistencias')],
      ['xG', sfVal(sf, 'xG')],
      ['Remates al arco / partido', sfVal(sf, 'Remates al arco')],
      ['Remates totales / partido', sfVal(sf, 'Remates totales')],
      ['Puntería %', sfVal(sf, 'Puntería %')],
      ['Regates completados / partido', sfVal(sf, 'Regates completados')],
      ['Regates exitosos %', sfVal(sf, 'Regates exitosos %')],
      ['Ocasiones creadas / partido', sfVal(sf, 'Ocasiones creadas')],
      ['Centros precisos %', sfVal(sf, 'Centros precisos %')],
      ['Faltas recibidas / partido', sfVal(sf, 'Faltas recibidas')],
      ['Pases clave / partido', sfVal(sf, 'Pases clave')],
      ['Duelos aéreos ganados %', sfVal(sf, 'Duelos aéreos ganados %')],
    ].filter(([, v]) => v);
    bars = [
      [sfNum(sf, 'Puntería %'), 'Puntería %', 100, '#c8f04a'],
      [sfNum(sf, 'Regates exitosos %'), 'Regates exitosos %', 100, '#4af0c8'],
      [sfNum(sf, 'Rating'), 'Rating', 10, '#4a90f0'],
    ];
  } else {
    // Genérico para posiciones no mapeadas
    especificas = [
      ['Goles', sfVal(sf, 'Goles')],
      ['Asistencias', sfVal(sf, 'Asistencias')],
      ['xG', sfVal(sf, 'xG')],
    ...
    bars = [[sfNum(sf, 'Rating'), 'Rating', 10, '#c8f04a']];
  }
 
  const validBars = bars.filter(([v]) => v !== null);
 
  return `
    <div class="obs-block" style="margin-bottom:12px">
      <div class="obs-title">Estadísticas Sofascore — ${sfVal(sf, 'Posición') || 'Temporada'}</div>
      <div class="detail-grid" style="margin-bottom:0">
        <div>
          <div style="font-size:11px;color:var(--text-3);font-family:var(--font-mono);text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px">Generales</div>
          ${generales.map(([l, v]) => `<div class="detail-row"><span class="detail-row-label">${l}</span><span class="detail-row-value">${v}</span></div>`).join('')}
          ${especificas.length ? `
            <div style="font-size:11px;color:var(--text-3);font-family:var(--font-mono);text-transform:uppercase;letter-spacing:.05em;margin:12px 0 8px">Específicas</div>
            ${especificas.map(([l, v]) => `<div class="detail-row"><span class="detail-row-label">${l}</span><span class="detail-row-value">${v}</span></div>`).join('')}
          ` : ''}
        </div>
        <div>
          ${validBars.length ? `
            <div style="font-size:11px;color:var(--text-3);font-family:var(--font-mono);text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px">Visual</div>
            ${validBars.map(([v, l, max, color]) => barHTML(l, v, max, color)).join('')}
          ` : ''}
        </div>
      </div>
    </div>`;
...
function populateSimSelect() {
  ...
  playerGroups.forEach((pg, i) => { sel.innerHTML += `<option value="${i}">${pg.nombre} — ${pg.pos}</option>`; });
...
function simScore(a, b, wStats) {
  ...
  const numScore = 1 - Math.sqrt(Math.pow(((a.nivelGeneral || 0) - (b.nivelGeneral || 0)) / 10, 2) + Math.pow(((a.nivelPotencial || 0) - (b.nivelPotencial || 0)) / 10, 2)) / Math.sqrt(2);
...
function renderSimilarity() {
  ...
  const scored = playerGroups.map((pg, i) => ({ pg, i, s: simScore(ref, pg, wStats) })).filter(x => x.i !== idx).sort((a, b) => b.s - a.s).slice(0, 6);
  res.innerHTML = `
    <div class="detail-card">
...
function getStatsStorage() { try { return JSON.parse(localStorage.getItem(STATS_KEY) || '[]'); } catch { return []; } }
function saveStatsStorage(data) { try { localStorage.setItem(STATS_KEY, JSON.stringify(data)); } catch (e) { console.warn('localStorage no disponible'); } }
function getStatVal(id) { const v = document.getElementById(id).value; return v === '' ? null : parseFloat(v); }
...
function guardarStats() {
  ...
  const entry = {
    id: Date.now(), jugador: nombre, partido: partido || '—', fecha: new Date().toLocaleDateString('es-AR'),
    goles: getStatVal('s-goles'), asistencias: getStatVal('s-asist'), remates: getStatVal('s-remates'),
    xg: getStatVal('s-xg'), regates: getStatVal('s-regates'), ocasiones: getStatVal('s-ocasiones'),
    pases: getStatVal('s-pases'), precision: getStatVal('s-precision'), duelos: getStatVal('s-duelos'),
    recuperaciones: getStatVal('s-recup'), intercepciones: getStatVal('s-interc'),
    despejes: getStatVal('s-despejes'), rating: getStatVal('s-rating'), minutos: getStatVal('s-minutos'),
  };
  const all = getStatsStorage(); all.push(entry); saveStatsStorage(all);
...
function limpiarStats() {
  ['s-goles','s-asist','s-remates','s-xg','s-regates','s-ocasiones','s-pases','s-precision','s-duelos','s-recup','s-interc','s-despejes','s-rating','s-minutos'].forEach(id => { document.getElementById(id).value = ''; });
}
...
function renderStatsView() {
  ...
  if (registros.length === 0) { cont.innerHTML = `<div class="empty-state"><div class="empty-icon">📊</div><p>${nombre ? `No hay estadísticas para "${nombre}".` : 'Ingresá un nombre arriba.'}</p></div>`; return; }
  const avg = key => { const vals = registros.map(r => r[key]).filter(v => v !== null && v !== undefined); return vals.length ? vals.reduce((a, b) => a + b, 0) / vals.length : null; };
...
function renderStatsHistory() {
  ...
  if (all.length === 0) { cont.innerHTML = '<div class="empty-state"><div class="empty-icon">📋</div><p>No hay estadísticas guardadas todavía.</p></div>'; return; }
  cont.innerHTML = [...all].reverse().map(e => `
    <div class="history-card">
      <div class="history-header">
        <div><div class="history-name">${e.jugador}</div><div class="history-match">${e.partido} · ${e.fecha}</div></div>
...
function eliminarStat(id) { saveStatsStorage(getStatsStorage().filter(e => e.id !== id)); renderStatsHistory(); }
